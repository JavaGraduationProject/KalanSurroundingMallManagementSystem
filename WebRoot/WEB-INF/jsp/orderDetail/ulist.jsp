...
<div class="panel admin-panel">
    ...
    <c:forEach items="${pagers.datas}" var="data" varStatus="l">
        <tr>
            ...
            <td>${data.total}</td>
            <td style="color: red">
                <c:if test="${data.itemOrder.status == 0}">
                    待发货
                </c:if>
                <c:if test="${data.itemOrder.status == 1}">
                    已取消
                </c:if>
                <c:if test="${data.itemOrder.status == 2}">
                    待收货
                </c:if>
                <c:if test="${data.itemOrder.status == 3}">
                    已收货
                </c:if>
                <c:if test="${data.itemOrder.status == 4}">
                    已退款
                </c:if>
            </td>
