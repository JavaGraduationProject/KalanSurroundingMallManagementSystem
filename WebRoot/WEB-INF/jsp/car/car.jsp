...
<script>
    ...
    //给小计赋值
    //查找包含商品价格和数量的元素的类来获取商品的单价和数量。计算商品的小计价格，并将结果显示在对应的元素中（".xzXj"）。
    (function cx() {
        for(var a=0;a<es;a++){
            var lt = $(".xzJg").eq(a).find("font").html();//商品的单价
            var num = $(".xzSl").eq(a).find("input").val();//商品的数量
            var xj = lt*num;
            $(".xzXj").eq(a).find("font").html(xj);//商品的小計
        }
    })();
    ...
    $(".xzWxz").click(function () {
        if($(this).hasClass("on")){//如果它已经有"class"属性为"on"，则会将其移除，否则会为它添加"class"属性为"on"
            $(this).removeClass("on");
        }else{
            $(this).addClass("on");
        }
        var ty = $(".xzWxz.on").length;//通过jQuery选择器找到所有"class"为"xzWxz"且有"class"为"on"的元素，并获取它们的数量。
        $(".sXd1").find("font").html(ty);//将数量更新到页面上的一个名为"sXd1"的元素的子元素"font"上。已經選擇商品數量
        if(ty==es){//如果已选中的元素数量与总数相等，则添加"class"属性"on"，否则移除。
            $(".ifAll").addClass("on");
        }else{
