...
public class ItemCategoryController extends BaseController {
    ...
    public String tj(ItemCategory itemCategory, Model model, HttpServletRequest request, HttpServletResponse response) {
        ...
        List<ItemCategory> list = itemCategoryService.listBySqlReturnEntity(sql);//查询一级类目
        List<Map<String,Object>> maps = new ArrayList<Map<String,Object>>();
        List<TjDto> res = new ArrayList<TjDto>();//定义了maps变量和res变量，分别用于存储后续处理过程中数据
        if (!CollectionUtils.isEmpty(list)){
            for (ItemCategory c : list){
                TjDto td = new TjDto();
                int tot = 0;//保存该类目下所有商品的销售总量
                ...
                //所有isDelete字段值为0且category_id_one字段值等于当前类目ID的记录
                if (!CollectionUtils.isEmpty(listBySqlReturnEntity)){
                    for (Item i : listBySqlReturnEntity){
                        tot+= i.getGmNum();//遍历查询到的商品列表，并将每个商品的销售数量加到tot变量中
                    }
                }
